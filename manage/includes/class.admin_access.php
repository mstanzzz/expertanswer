...
class AdminAccess
{
	...
	function AdminAccess(){
		...
		$this->customers_orders_level = $this->getCustomersOrdersLevel();	
	...
	public function getAdminGroupIDs(){
		...
		if(!isset($_SESSION['admin_access']['admin_group_ids'])){
			...
			$sql = "SELECT admin_group_id
					...
					WHERE profile_id = '".$lgn->getProfileId()."'";					
	...
	public function getCustomersOrdersLevel() {
		if(!isset($_SESSION['admin_access']['customers_orders_level'])){
			$_SESSION['admin_access']['customers_orders_level'] = 0;
			foreach($_SESSION['admin_access']['admin_group_ids'] as $val){
				$dbCustom = new DbCustom();
				$db = $dbCustom->getDbConnect(USER_DATABASE);
				$sql = "SELECT admin_section_level
				...
				if($result->num_rows > 0){
					$object = $result->fetch_object();
					if($_SESSION['admin_access']['customers_orders_level'] < $object->admin_section_level){
						 $_SESSION['admin_access']['customers_orders_level'] = $object->admin_section_level;
					}
				}
			}			
		}
		return $_SESSION['admin_access']['customers_orders_level'];
	}
